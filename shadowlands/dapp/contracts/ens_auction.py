...
########
    ...
    def place_bid(self, name, bidding_address, bid_amt_ether, secret):
        if bidding_address.startswith('0x'):
            bidding_address = decode_hex( bidding_address.replace('0x','') )
        else:
            raise Exception("place_bid() expects bidding_address to be a normal ethereum address string starting with 0x")
